---
title: 📢 1st Digital Agreement for families and technology in Forlì
summary: 
date: 2026-04-29
tags:
  - patto digitale
  - digital agreement
  - correct use of technologies
image:
  caption: 'A snapshot of the newspaper coverage'
authors:
  - admin
---

## Overview

On the evening of the 29th of April, the efforts of over 40 parents from the 
city of Forlì, concerned with a healthy use of technology by young people (from 
social media to smart devices), culminated with the signature of the **first**
_Patto Digitale_ of the city of Forlì (Italy).

The agreement includes a number of guidelines and commitments to introduce 
kids to technology in the right way and at the right time (e.g, from zero use 
of smart devices before 2 years old up to proper boundaries for teenagers).

Besides taking part in the shaping of part of the guidelines, I was kindly 
invited to offer the dissemination talk 
*Un breve percoso all’interno dell’IA generativa* (A brief path into generative 
AI), in which I explained the technology behind the pervasive services that are 
currently widely known as artificial intelligence: natural language processing
and, in particular, large language models.

## Go further

- 📰 [**News coverage of the signature](https://www.ilrestodelcarlino.it/forli/cronaca/smartphone-tablet-minori-guida-genitori-regole-eb6ocg60)
- 📚 [**Check the slides**](/uploads/pe/barroncedeno-pattodigitale26.pdf)
